<section class="container mx-auto p-6 font-mono">
    <div class="w-full flex mb-4 p-2 justify-end">
        <form class="flex space-x-4 shadow bg-white rounded-md m-2 p-2">
            <div class="p-1 flex items-center">
                <label for="tmdb_id_g" class="block text-sm font-medium text-gray-700 mr-4 mr-4">Series Name</label>
                <div class="relative rounded-md shadow-sm">
                    <input wire:model="name" id="series-name" name="name"
                        class="px-3 py-2 border border-gray-300 rounded" placeholder="Series Name" />
                </div>
            </div>
            <div class="p-1">
                <button type="button" wire:click="createSeries"
                    class="inline-flex items-center justify-center py-2 px-4 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-gray-800 hover:bg-green-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-green-700 transition duration-150 ease-in-out disabled:opacity-50">
                    <span>Create</span>
                </button>
            </div>
        </form>
                    ...
                    @foreach ($series as $serie)
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3 text-sm border">
                                ...
                                <x-m-button wire:click="deleteSeries({{ $serie->id }})"
    ...
    <x-jet-dialog-modal wire:model="showSeriesModal">
        <x-slot name="title">Update Series</x-slot>
        <x-slot name="content">
        ...
        <x-slot name="footer">
            <x-m-button wire:click="closeSeriesModal" class="bg-gray-600 hover:bg-gray-800 text-white">Cancel
            </x-m-button>
